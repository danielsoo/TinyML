...
def main():
    ...
    if not args.skip_analysis:
        ...
        # Explicit models: Keras (baseline) + Original TFLite + Compressed (so report shows all 3)
        keras_path = Path("models/global_model.h5")
        if not keras_path.exists():
            keras_path = Path("src/models/global_model.h5")
        models_arg = []
        if keras_path.exists():
            models_arg.extend(["Keras:" + str(keras_path)])
        models_arg.extend([
    ...
    if not args.skip_ratio_sweep:
        ...
        if not ratio_model_path.exists():
            ...
            # Where to save the report: prefer current run (if analysis ran this session)
            last_run_file = Path("data/processed/runs/.last_run_id")
            if current_runs_dir is not None:
                # Analysis ran this session → save eval to same run folder
                report_dir = current_runs_dir / "eval"
                report_dir.mkdir(parents=True, exist_ok=True)
                report_path = report_dir / "ratio_sweep_report.md"
                eval_report_dir = report_dir
                print(f"  📁 Eval report dir: {report_dir}")
            elif args.model and get_eval_dir_for_model(ratio_model) is not None:
                # Eval-only with --model under runs/ → save to model's run
                report_dir = get_eval_dir_for_model(ratio_model)
                report_dir.mkdir(parents=True, exist_ok=True)
                report_path = report_dir / "ratio_sweep_report.md"
                eval_report_dir = report_dir
                ...
                # Eval-only run: no analysis yet → data/processed/eval/<timestamp>/
    ...
    if not args.skip_viz:
        ...
        if not results_json.exists():
            fallback_dir = Path("data/processed/runs/latest/analysis")
            fallback_json = fallback_dir / "compression_analysis.json"
            if fallback_json.exists():
                analysis_dir, results_json = fallback_dir, fallback_json
                print(f"  (Using fallback path: {results_json})")
        viz_cmd = [
