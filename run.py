...
"""
import os
# CRITICAL: Set BEFORE any imports that might use TensorFlow/Keras
    ...
    if not args.skip_compression:
        ...
        if run_cfg.get("federated", {}).get("use_qat", False):
            ...
            if not qat_tflite.exists():
                print("\n" + "=" * 80)
                print("❌ PIPELINE ABORTED: use_qat is True but QAT TFLite file is missing.")
                print(f"   Expected: {qat_tflite.resolve()}")
                print("=" * 80 + "\n")
                return 1
        all_success = all_success and success
    else:
