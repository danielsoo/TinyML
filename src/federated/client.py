...
def simulate_clients(config: dict = None):
    ...
    # Print configuration version and key settings
    version = config.get("version", "unknown")
    print("\n" + "="*80)
    print(" "*25 + f"🚀 FEDERATED LEARNING {version}")
    print("="*80)
    ...
    print(f"  • Num rounds: {fed_cfg.get('num_rounds', 100)}")
    print(f"\n[Federated Training]")
    print(f"  • Local epochs: {fed_cfg.get('local_epochs', 1)}")
    print(f"  • Batch size: {fed_cfg.get('batch_size', 32)}")
    print(f"  • Learning rate: {fed_cfg.get('learning_rate', 0.001)}")
    print(f"  • LR decay: {fed_cfg.get('lr_decay_type', 'none')}")
    print(f"  • Server momentum: {fed_cfg.get('server_momentum', 0.9)}")
    print(f"  • Server LR: {fed_cfg.get('server_learning_rate', 1.0)}")
    print(f"\n[Loss Configuration]")
    print(f"  • Use class weights: {fed_cfg.get('use_class_weights', False)}")
    print(f"  • Use focal loss: {fed_cfg.get('use_focal_loss', False)}")
    print(f"  • Focal loss alpha: {fed_cfg.get('focal_loss_alpha', 0.75)}")
    print(f"  • Use distillation: {fed_cfg.get('use_distillation', False)}")
    print(f"\n[Data Settings]")
    print(f"  • Max samples: {data_cfg.get('max_samples', 'unlimited')}")
    print(f"  • Balance ratio: {data_cfg.get('balance_ratio', 1.0)}")
    print(f"  • Use SMOTE: {data_cfg.get('use_smote', False)}")
    print(f"\n[Compression]")
    print(f"  • Use QAT: {fed_cfg.get('use_qat', False)}")
    print("="*80 + "\n")
...
def main(save_path: str = "src/models/global_model.h5", config_path: str = None):
    ...
    # Replace NaN with None so json.dump works (nan is not JSON-serializable)
    def _sanitize(obj):
    ...
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "rounds": _sanitize(metrics_history),
