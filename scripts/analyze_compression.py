...
class CompressionAnalyzer:
    ...
    def _generate_markdown_report(self, df: pd.DataFrame, output_path: Path):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"| **Adversarial training (AT) enabled** | {at_cfg.get('enabled', '-')} |\n")
            f.write(f"| **AT attack** | {at_cfg.get('attack', '-')} |\n")
            f.write(f"| **AT epsilon** | {at_cfg.get('epsilon', '-')} |\n")
            f.write(f"| **AT epochs** | {at_cfg.get('epochs', '-')} |\n")
            f.write(f"| **AT adv_ratio** | {at_cfg.get('adv_ratio', '-')} |\n")
            f.write(f"| **AT pgd_steps** | {at_cfg.get('pgd_steps', '-')} |\n")
            f.write(f"| **AT pgd_alpha** | {at_cfg.get('pgd_alpha', '-')} |\n")
            f.write(f"| **PGD top-N models** | {eval_cfg.get('pgd_top_n', eval_cfg.get('fgsm_top_n', '-'))} |\n")
            f.write(f"| **PGD metric** | {eval_cfg.get('pgd_metric', eval_cfg.get('fgsm_metric', '-'))} |\n")
            f.write("\n")
            f.write("**전체 실험 설정:** 동일 run 디렉터리의 `run_config.yaml` 및 `experiment_record.md` 참조.\n\n")
