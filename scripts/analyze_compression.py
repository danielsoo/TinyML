...
class CompressionAnalyzer:
    ...
    def evaluate_model(
        ...
        if model_path.endswith(".h5"):
            ...
            if self.num_classes <= 2:
                ...
                th = float(eval_cfg.get("prediction_threshold", 0.5))
        ...
        prec_kw = {"average": "binary", "pos_label": 1} if self.num_classes <= 2 else {"average": "weighted"}
        precision = precision_score(self.y_test, y_pred, zero_division=0, **prec_kw)
        recall = recall_score(self.y_test, y_pred, zero_division=0, **prec_kw)
        if self.num_classes <= 2:
            f1 = f1_score(self.y_test, y_pred, average="binary", pos_label=1, zero_division=0)
        else:
    ...
    @staticmethod
    def _config_table_markdown(config: dict) -> str:
        """Build 'Run / Training Configuration' table from config dict."""
        data_cfg = config.get("data", {})
        fed_cfg = config.get("federated", {})
        model_cfg = config.get("model", {})
        eval_cfg = config.get("evaluation", {})
        max_s = data_cfg.get("max_samples")
        lines = [
            "| Item | Value |",
            "|------|-------|",
            f"| **Data** | {data_cfg.get('name', '-')} |",
            f"| **Data path** | {data_cfg.get('path', data_cfg.get('data_path', '-'))} |",
            f"| **Max samples** | {max_s if max_s is not None else 'unlimited'} |",
            f"| **Balance ratio** (normal:attack) | {data_cfg.get('balance_ratio', '-')} |",
        ...
        if eval_cfg.get("prediction_threshold") is not None:
            idx = next(i for i, s in enumerate(lines) if "Focal loss alpha" in s)
            lines.insert(idx + 1, f"| **Prediction threshold** | {eval_cfg.get('prediction_threshold')} |")
        return "\n".join(lines)
    ...
    def _generate_markdown_report(self, df: pd.DataFrame, output_path: Path):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            # Run / Training Configuration (from config used for this run)
            f.write("## Run / Training Configuration\n\n")
            f.write(self._config_table_markdown(self.config))
            f.write("\n\n")
...
def backfill_config_section(runs_dir: Path) -> int:
    """
    Add 'Run / Training Configuration' to existing compression_analysis.md
    that don't have it, using run_config.yaml from each run directory.
    Returns number of files updated.
    """
    updated = 0
    for report_path in runs_dir.rglob("analysis/compression_analysis.md"):
        content = report_path.read_text(encoding="utf-8")
        if "## Run / Training Configuration" in content:
            continue
        run_dir = report_path.parent.parent  # analysis -> run dir
        ...
        if not config:
            continue
        config_section = "## Run / Training Configuration\n\n" + CompressionAnalyzer._config_table_markdown(config) + "\n\n"
        idx = content.find("| **Generated** |")
        if idx == -1:
            continue
        end_line = content.find("\n", idx)
        insert_after = content.find("\n", end_line + 1)
        if insert_after == -1:
            insert_after = len(content)
        new_content = content[: insert_after + 1] + config_section + content[insert_after + 1 :]
        report_path.write_text(new_content, encoding="utf-8")
        updated += 1
        print(f"Backfilled config: {report_path}")
    return updated
...
def main():
    ...
    parser.add_argument(
        "--backfill",
        action="store_true",
        help="Add Run/Training Configuration to existing compression_analysis.md that lack it (using run_config.yaml).",
    )
    ...
    if args.backfill:
        ...
        if not runs_dir.exists():
            ...
            return
        n = backfill_config_section(runs_dir)
        print(f"Backfilled {n} report(s).")
        return
    ...
    if args.models is None:
        ...
        # Keras first (baseline), then TFLite Original, then Compressed
        keras_path = "models/global_model.h5"
        if not Path(keras_path).exists() and Path("src/models/global_model.h5").exists():
            keras_path = "src/models/global_model.h5"
        args.models = [
            f"Keras:{keras_path}",
            "Original (TFLite):models/tflite/saved_model_original.tflite",
