...
def plot_fl_history(
    ...
    mean_acc_pct = [_safe_float(r.get("accuracy_pct"), 0) for r in rounds_data]
    ...
    for i in range(num_clients):
        ...
        if key_pct in rounds_data[0] and i < len(rounds_data[0][key_pct]):
            device_pct = [_safe_float(r[key_pct][i] if i < len(r.get(key_pct, [])) else 0, 0) for r in rounds_data]
