"""
test_quantization.py — TFLite Quantization for Standard MLP (Normal Path)

Loads the pruned student model from test_pruning.py,
exports to TFLite with INT8 quantization.

Pipeline: Pruned model → TFLite (float) → TFLite (INT8 quantized)

Usage:
    python test/normal/test_quantization.py
    python test/normal/test_quantization.py --pruning-results models/cicids/pruning_results.json
"""
import os
...
def main():
    parser = argparse.ArgumentParser(description="Quantization — Standard MLP Student (nets.py)")
    parser.add_argument('--pruning-results', type=str, default='models/cicids/pruning_results.json',
                        help='Path to pruning_results.json from test_pruning.py')
    ...
    print("\n" + "=" * 70)
    print("TEST_QUANTIZATION: TFLite Export + INT8 Quantization")
    print("  Input: pruned_standard.h5 (from test_pruning.py)")
    print("  Normal path: FL(mlp) → Distill → Prune → Quantize")
    print("=" * 70)
    ...
    # --- Export TFLite (float32, no quantization) ---
    print("\n" + "=" * 70)
    print("STEP 3: TFLITE EXPORT (float32)")
    print("=" * 70)

    float_path = str(output_dir / "standard_float.tflite")
    ...
    quant_path = str(output_dir / "standard_int8.tflite")
    ...
    results = {
        ...
        "method": "quantization_standard_mlp",
